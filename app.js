...
function initializeWebSocket() {
  voidPool = new ethers.Contract(VOID_POOL_ADDRESS, UNISWAP_V3_POOL_ABI, wsProvider);
  voidTokenWS = new ethers.Contract(VOID_CONTRACT_ADDRESS, ERC20_ABI, wsProvider);
...
async function handleTransfer(from, to, value, event) {
  console.log(`[${new Date().toISOString()}] Received Transfer event:`, JSON.stringify({ from, to, value: value.toString() }, null, 2));
...
async function handleSwapEvent(event) {
  try {
    console.log(`[${new Date().toISOString()}] Received Swap event:`, JSON.stringify(event, null, 2));
